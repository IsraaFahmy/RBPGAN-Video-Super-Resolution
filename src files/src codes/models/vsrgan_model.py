...
class VSRGANModel(VSRModel):
    ...
    def set_networks(self):
        ...
        if self.is_train:
            ...
            for p in self.net_D.parameters():
                total_d_param += torch.numel(p)
            base_utils.log_info('Number of parameters in discriminator: ' + str(total_d_param))
        ...
        if load_path_G:
            ...
            for p in self.net_G.parameters():
                total_g_param += torch.numel(p)
            #total_g_param = sum(p.numel for p in self.net_G.parameters())
            base_utils.log_info('Number of parameters in generator: ' + str(total_g_param))



    def set_criterions(self):
        #losses are mentioned in the train.yaml file with type and weight of each
        ...
        self.fm_crit = define_criterion(self.opt['train'].get('feature_matching_crit')) #not mentioned

        # gan criterion
        self.gan_crit = define_criterion(self.opt['train'].get('gan_crit'))

        self.L1_crit = define_criterion(self.opt['train'].get('L1_crit'))
    ...
    def train(self):
        ...
        #we got the discriminator prediction for the fake (generated) data and for the gt data
        ...
        if update_policy == 'adaptive':
            ...
            distance = (logged_real_pred_D - logged_fake_pred_D).item() #set update to true if fake prediction is close to the gt prediction (difference is the threshold)
            upd_D = distance < self.opt['train']['discriminator']['update_threshold']
        else:
            upd_D = True

        if upd_D: #get loss and update weights
            self.cnt_upd_D += 1.0
        ...
        if update_policy == 'adaptive':
            ...
            self.log_dict['n_upd_D'] = self.cnt_upd_D #number of updates happened to D
        ...
        #every loss contributes with specific weight in the total loss
        #loss_G is the total of all generator losses
        #losses are in yaml file and their functions are set in set_criterions above

        #either l1 or pixel 

        # L1 loss
        if self.L1_crit is not None:
            L1_w = self.opt['train']['L1_crit'].get('weight', 1)
            loss_L1_G = L1_w * self.L1_crit(hr_data, gt_data)
            loss_G += loss_L1_G
            self.log_dict['l_L1_G'] = loss_L1_G.item()
        ...
        if self.warp_crit is not None:
            lr_curr = net_G_output_dict['lr_curr'] #from 1 to last 
            lr_prev = net_G_output_dict['lr_prev'] #from 0 to before last
            lr_flow = net_G_output_dict['lr_flow'] # the flow between every frame and its next one (between lr_curr and lr_prev )
            lr_warp = net_utils.backward_warp(lr_prev, lr_flow) #warps every frame from 0 to before last with the flow between it and its next frame

            warp_w = self.opt['train']['warping_crit'].get('weight', 1)
            loss_warp_G = warp_w * self.warp_crit(lr_warp, lr_curr) #difference between frames with and without warping
            loss_G += loss_warp_G
